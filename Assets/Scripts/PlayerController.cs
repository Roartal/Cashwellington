...
public class PlayerController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (detectVaultObject.Obstruction && !detectVaultObstruction.Obstruction && !CanVault && !IsParkour && !WallRunning
            && (Input.GetKey(KeyCode.Space) || !rbfps.IsGrounded()) && Input.GetAxisRaw("Vertical") > 0f)
        ...
        if (CanVault)
        {
           ...
           // cameraAnimator.CrossFade("Vault",0.1f);
            cameraAnimator.SetTrigger("Vault");
        }
        ...
        if (CanClimb)
        {
            ...
            print("climbing");
            // cameraAnimator.CrossFade("Vault",0.1f);
            cameraAnimator.SetTrigger("Climb");
        }
        ...
        //Wallrun
        if (DetectWallL.Obstruction && !rbfps.IsGrounded() && !IsParkour && canwallrun) // if detect wall on the left and is not on the ground and not doing parkour(climb/vault)
        {
        ...
        if (DetectWallR.Obstruction && !rbfps.IsGrounded() && !IsParkour && canwallrun) // if detect wall on thr right and is not on the ground
        {
